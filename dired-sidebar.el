...
(defcustom dired-sidebar-use-uniquify nil
  "Whether dired-sidebar should use uniquify.el to uniquify buffer names."
  :type 'boolean
  :group 'dired-sidebar)
...
(defun dired-sidebar-find-existing (root)
  (when-let ((found (seq-find
                     (lambda (entry)
                       (let ((dir (car entry))
                             (b (cdr entry)))
                         (and (string-equal dir root)
                              (buffer-live-p b)
...
(defun dired-sidebar-get-or-create-buffer (root)
  ...
  (let ((name (dired-sidebar-buffer-name root)))
    (if-let* ((existing-buffer (dired-sidebar-find-existing root)))
        existing-buffer
